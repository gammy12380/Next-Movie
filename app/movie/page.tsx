...
export default function TV() {
	...
	const fetchTVData = useCallback(async () => {
		...
		try {
			const res = await fetchAPI<MovieCommon<MovieList>>(discoverMovieUrl, { queryParams })
			if (init) setTvList([])
		...
		<div className="space-y-4 py-4">
			...
			<div className="max-sm:p-4 py-4 px-8">
				<div className="flex justify-between">
					<h4 className="text-white text-lg max-sm:text-sm">電影</h4>
				...
				<MovieSection isLoading={sectionLoading} layout="grid" total={total} type="movie" list={tvList} onSearchMore={searchMore} />
			</div>
		</div>
	)
